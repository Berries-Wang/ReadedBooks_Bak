...
+ Java 堆(Java Heap)
  ...
  - 内存调整参数
    ...
    + -Xms 设定程序启动时占用内存大小
+ 方法区（Method Area）
  - 方法区(别名:Non-heap:非堆) 也被称为”永久代“。与Java堆一样，也是各个线程共享的内存区域。用于存储已经被JVM加载的类信息、常量(jdk7已放入了java heap中)、静态变量(jdk7已放入了java heap中)、即时编译器编译后的代码等数据。在HotSpot虚拟机中，将GC的分代收集拓展到方法区，或者说是使用永久代来实现方法区。那么，HotSpot的垃圾收集器就可以像管理Java堆一样来管理这一部分的内存。
  - 注意:
